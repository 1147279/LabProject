...
int main ()
{
  ...
  milli = 400000;
	...
	while(1)
	{
		inputCourage.open("compressedbloomFinalCourage.txt");
		if (inputCourage)
		{
			break;
		}
...
usleep(milli);
	...
	inputCourage.open("compressedbloomFinalCourage.txt");
	inputRalph.open("bitstringOUTRalph.txt");

	string RalphBloom = "";
	inputRalph >> RalphBloom;
	cout << "RalphBloom size: "<< RalphBloom.size()<<endl;


	string compressedCouragebloom,Couragebloom;
	compressedCouragebloom="";
	Couragebloom = "";
	inputCourage >> compressedCouragebloom;
	cout << "compressedCouragebloom size: "<< compressedCouragebloom.size()<<endl;



	int sz = compressedCouragebloom.size()/wordlength;

	for (int i = 0 ; i < sz ; i++)
	{
		Couragebloom = decompressor(i,wordlength,compressedCouragebloom) + Couragebloom;
	}

	cout << "Couragebloom size: "<< Couragebloom.size()<<endl;
	string resBloom = logicalEnd(RalphBloom,Couragebloom);
	...
	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalRichard.txt",ofstream::out);
	...
	system("./sendBloomRalph.sh");

	system("sqlite3 -header -csv \'pressure.db\' \'select * from PRESSURE; \' > outPressure.csv");

	ifstream pressurecsv;
	pressurecsv.open("outPressure.csv");
	...
	participators.open("ForJoinRalph.csv");

	string tempString= "";
  setBloomFilter("resultingBitString.txt",decompResultBloom);

	string firstline;
	pressurecsv >> firstline;

	while (pressurecsv.good())
	{
		getline(pressurecsv,ID,',');
		getline(pressurecsv,year,',');
		getline(pressurecsv,month,',');
		getline(pressurecsv,day,',');
		getline(pressurecsv,hour,',');
		getline(pressurecsv,minute,',');
		getline(pressurecsv,second,',');
		getline(pressurecsv,location,',');
		getline(pressurecsv,pressure,'\n');
