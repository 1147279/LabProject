...
int main ()
{
  ...
  milli = 100000;
	...
	input.open("bitstringOUTRalph.txt");
	...
	bloom = "";
	input >> bloom;
	cout << "compressedtempbloom size: "<< bloom.size()<<endl;




	string resBloomCompd = compressString(bloom,wordlength);

	ofstream outputcomp;
	outputcomp.open("resultingBitStringCompressedRalph.txt",ofstream::out);
