...
int main ()
{
  ...
  milli = 400000;
	...
	while(1)
	{
		inputMac.open("compressedbloomFinalMac.txt");
		if (inputMac)
		{
			break;
		}
...
usleep(milli);
	...
	inputMac.open("compressedbloomFinalMac.txt");
	inputMurphy.open("bitstringOUTMurphy.txt");

	string MurphyBloom = "";
	inputMurphy >> MurphyBloom;
	cout << "MurphyBloom size: "<< MurphyBloom.size()<<endl;


	string compressedMacbloom,Macbloom;
	compressedMacbloom="";
	Macbloom = "";
	inputMac >> compressedMacbloom;
	cout << "compressedMacbloom size: "<< compressedMacbloom.size()<<endl;



	int sz = compressedMacbloom.size()/wordlength;

	for (int i = 0 ; i < sz ; i++)
	{
		Macbloom = decompressor(i,wordlength,compressedMacbloom) + Macbloom;
	}

	cout << "Macbloom size: "<< Macbloom.size()<<endl;
	string resBloom = logicalEnd(MurphyBloom,Macbloom);
	...
	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalJohn.txt",ofstream::out);
	...
	system("./sendBloomMurphy.sh");

	system("sqlite3 -header -csv \'humidity.db\' \'select * from HUMIDITY; \' > outHumidity.csv");

	ifstream humiditycsv;
	humiditycsv.open("outHumidity.csv");
	...
	participators.open("ForJoinMurphy.csv");
  ...
  setBloomFilter("resultingBitString.txt",decompResultBloom);
	...
	system("./sendCSVForJoinMurphy.sh");
	//cout << str << endl;
