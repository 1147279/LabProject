...
int main ()
{
  ...
  milli = 200000;
	...
	ifstream yt;
	yt.open("compressedbloomFinalStorm.txt");
	string varsize ;
	yt >> varsize;
	cout << "Size of compressed storm bit string: "<< varsize.size()<<endl;

	system("./sendBloomFirstStorm.sh");
	...
	inputres.open("compressedbloomFinalAudrey.txt");
	inputStorm.open("bitstringOUTStorm.txt");

	string resbloom = "";
	string in,compressedresbloom,StormBloom;

	inputres >> compressedresbloom;
	cout << "compressedresbloom size: "<< compressedresbloom.size()<<endl;


	inputStorm >> StormBloom;
	...
	string finalResBloom = logicalEnd(resbloom,StormBloom);
	...
	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalAudrey.txt");
...
///WWWWWWWWWWWWWAAAAAAAAAAAAAAATTTTTTTTTTTCCCCCCCCCHHHHHHHHHHHHHHH
	...
	system("./sendBloomStorm.sh");

	system("sqlite3 -header -csv \'wind.db\' \'select * from WIND; \' > outWind.csv");

	ifstream windcsv;
	windcsv.open("outWind.csv");
	...
	participators.open("ForJoinStorm.csv");
  ...
  setBloomFilter("FinalresultingBitString.txt",decompResultBloom);
	...
	{
		getline(windcsv,ID,',');
		getline(windcsv,year,',');
		getline(windcsv,month,',');
		getline(windcsv,day,',');
		getline(windcsv,hour,',');
		getline(windcsv,minute,',');
		getline(windcsv,second,',');
		getline(windcsv,location,',');
		getline(windcsv,wind,'\n');
