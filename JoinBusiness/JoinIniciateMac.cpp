...
int main ()
{
  ...
  milli = 200000;
	...
	ifstream yt;
	yt.open("compressedbloomFinalMac.txt");
	string varsize ;
	yt >> varsize;
	cout << "Size of compressed storm bit string: "<< varsize.size()<<endl;

	system("./sendBloomFirstMac.sh");

	cout << "waiting for new bloom filter..." << endl;

	while(1)
	{
		inputres.open("compressedbloomFinalJohn.txt");
		if (inputres)
		{
			break;
		}
	...
	inputres.open("compressedbloomFinalJohn.txt");
	inputMac.open("bitstringOUTMac.txt");

	string resbloom = "";
	string in,compressedresbloom,MacBloom;
	...
	string finalResBloom = logicalEnd(resbloom,MacBloom);
	...
	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalJohn.txt");
...
///WWWWWWWWWWWWWAAAAAAAAAAAAAAATTTTTTTTTTTCCCCCCCCCHHHHHHHHHHHHHHH
	...
	system("./sendBloomMac.sh");

	system("sqlite3 -header -csv \'temperature.db\' \'select * from WEATHER; \' > outTemperature.csv");

	ifstream temperaturecsv;
	temperaturecsv.open("outTemperature.csv");
	...
	participators.open("ForJoinMac.csv");
