...
int main ()
{
  ...
  milli = 100000;
	...
	cout << "Join Mac Received" << endl;

	while(1)
	{
		infile.open("ForJoinMurphy.csv");
		if (infile)
		{
			break;
		}
...
//ROUND 2
  ...
  */
  ...
  ifstream firstcsv;
	firstcsv.open("ForJoinMac.csv");


  string decompResultBloom = resBloom2;

  ofstream participators;
	participators.open("ForJoinMacReduced.csv");
  ...
  firstcsv.close();
	firstcsv.open("ForJoinMurphy.csv");



  participators.close();
	participators.open("ForJoinMurphyReduced.csv");
