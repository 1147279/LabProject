...
int main ()
{
  ...
  milli = 200000;
	int wordlength =14;
	ifstream inputres;
	ifstream inputCourage;

	cout << "sending bloom filter..." << endl;

	ifstream yt;
	yt.open("compressedbloomFinalCourage.txt");
	string varsize ;
	yt >> varsize;
	cout << "Size of compressed storm bit string: "<< varsize.size()<<endl;

	system("./sendBloomFirstCourage.sh");

	cout << "waiting for new bloom filter..." << endl;

	while(1)
	{
		inputres.open("compressedbloomFinalRichard.txt");
		if (inputres)
		{
			break;
		}
	...
	inputres.open("compressedbloomFinalRichard.txt");
	inputCourage.open("bitstringOUTCourage.txt");

	string resbloom = "";
	string in,compressedresbloom,CourageBloom;

	inputres >> compressedresbloom;
	cout << "compressedresbloom size: "<< compressedresbloom.size()<<endl;


	inputCourage >> CourageBloom;

	cout << "CourageBloom size: "<< CourageBloom.size()<<endl;
	...
	string finalResBloom = logicalEnd(resbloom,CourageBloom);
	...
	output.close();
	string resBloomCompd = compressString(finalResBloom,wordlength);

	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalRichard.txt");

///WWWWWWWWWWWWWAAAAAAAAAAAAAAATTTTTTTTTTTCCCCCCCCCHHHHHHHHHHHHHHH
	...
	system("./sendBloomCourage.sh");

	system("sqlite3 -header -csv \'radiation.db\' \'select * from RADIATION; \' > outRadiation.csv");

	ifstream radiationcsv;
	radiationcsv.open("outRadiation.csv");
	...
	participators.open("ForJoinCourage.csv");

	string tempString= "";
  setBloomFilter("FinalresultingBitString.txt",decompResultBloom);

	string firstline;
	radiationcsv >> firstline;

	while (radiationcsv.good())
	{
		getline(radiationcsv,ID,',');
		getline(radiationcsv,year,',');
		getline(radiationcsv,month,',');
		getline(radiationcsv,day,',');
		getline(radiationcsv,hour,',');
		getline(radiationcsv,minute,',');
		getline(radiationcsv,second,',');
		getline(radiationcsv,location,',');
		getline(radiationcsv,radiation,'\n');
	...
	system("sudo service ssh start");
	system("./sendCSVForJoinCourage.sh");
	//cout << str << endl;
