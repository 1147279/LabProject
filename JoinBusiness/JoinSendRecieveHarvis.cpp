...
int main ()
{
  ...
  milli = 100000;
	...
	while(1)
	{
		inputStorm.open("compressedbloomFinalStorm.txt");
		if (inputStorm)
		{
			break;
		}
...
usleep(milli);
	...
	inputStorm.open("compressedbloomFinalStorm.txt");
	inputHarvis.open("bitstringOUTHarvis.txt");

	string HarvisBloom = "";
	inputHarvis >> HarvisBloom;
	cout << "HarvisBloom size: "<< HarvisBloom.size()<<endl;


	string compressedStormbloom,Stormbloom;
	compressedStormbloom="";
	Stormbloom = "";
	inputStorm >> compressedStormbloom;
	cout << "compressedStormbloom size: "<< compressedStormbloom.size()<<endl;



	int sz = compressedStormbloom.size()/wordlength;

	for (int i = 0 ; i < sz ; i++)
	{
		Stormbloom = decompressor(i,wordlength,compressedStormbloom) + Stormbloom;
	}

	cout << "Stormbloom size: "<< Stormbloom.size()<<endl;
	string resBloom = logicalEnd(HarvisBloom,Stormbloom);
	...
	ofstream outputcomp;
	outputcomp.open("compressedbloomFinalAudrey.txt",ofstream::out);
	...
	system("./sendBloomHarvis.sh");

	system("sqlite3 -header -csv \'precipitation.db\' \'select * from PRECIPITATION; \' > outPrecip.csv");

	ifstream precipitationcsv;
	precipitationcsv.open("outPrecip.csv");
	...
	participators.open("ForJoinHarvis.csv");

	string tempString= "";
  setBloomFilter("resultingBitString.txt",decompResultBloom);

	string firstline;
	precipitationcsv >> firstline;

	while (precipitationcsv.good())
	{
		getline(precipitationcsv,ID,',');
		getline(precipitationcsv,year,',');
		getline(precipitationcsv,month,',');
		getline(precipitationcsv,day,',');
		getline(precipitationcsv,hour,',');
		getline(precipitationcsv,minute,',');
		getline(precipitationcsv,second,',');
		getline(precipitationcsv,location,',');
		getline(precipitationcsv,precipitation,'\n');
	...
	system("./sendCSVForJoinHarvis.sh");
	//cout << str << endl;
