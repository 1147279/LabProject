...
int main(int argc, char *argv[])
{
  ...
  ifstream input_A;
  input_A.open("Lteble1.txt", ifstream::in);

  ifstream input_B;


  ofstream outFile;
  outFile.open("Lteble3.txt",ofstream::out);
  ...
  while (input_A >> kv_pair_A)
  {
    ...
    input_B.open("Lteble2.txt", ifstream::in);
    while ((input_B >> kv_pair_B)&&(!isFound))
    {
      ...
      }else
      {
        ...
        join_kv = hashJoin(key_A, kv_pair_A, tmp_kvpair);
        //cout << join_kv << '\n';
        isFound = false;
  ...
  input_A.open("Lteble2.txt", ifstream::in);
  ...
  while (input_A >> kv_pair_A)
  {
    ...
    key_A= keyExtraction(delimiter, kv_pair_A);       //Extracting the key from the key-value pair read in from Table A.
    input_B.open("Lteble1.txt", ifstream::in);
    while ((input_B >> kv_pair_B)&&(!isFound))
    {
